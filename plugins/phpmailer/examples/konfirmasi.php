<?php
  $sql_konfirmasi_terupdate=mysqli_query($conn, "SELECT * FROM konfirmasi_order WHERE kd_konfirmasi='$_GET[id]'");
  $data_konfirmasi_terupdate=mysqli_fetch_assoc($sql_konfirmasi_terupdate);
  $sql_user=mysqli_query($conn, "SELECT * FROM user left join orders on user.kd_user=orders.kd_user where orders.kd_order='$data_konfirmasi_terupdate[kd_order]'");
  $data_user=mysqli_fetch_assoc($sql_user);
  if ($data_konfirmasi_terupdate[status]=="valid") {
    $atas="Sukses";
    $tengah="Selamat! Pembayaran untuk Invoce no <strong>#".$data_konfirmasi_terupdate[kd_order]."</strong> telah berhasil. Terima kasih atas kepercayaan kamu berbelanja di bengkelikan.com.";
    $bawah="Untuk memantau status transaksimu, kunjungi halaman detail pesanan. Jika pesanan tidak dikirimkan, dana akan dikembalikan 100%";
    $tombol='<a href="'.$base_url.'/user/detail-pesanan='.$data_konfirmasi_terupdate[kd_order].'/'.$data_user[kd_user].'" class="btn btn-primary">Lihat Detail Pesanan</a>';
  }else{
    $atas="Tidak Valid";
    $tengah="Maaf! Pembayaran untuk Invoce no <strong>#".$data_konfirmasi_terupdate[kd_order]."</strong> tidak valid. cek kembali konfirmasi pembayaran anda dengan benar dan lakukan konfirmasi pembayaran lagi.";
    $bawah="Untuk memantau status transaksimu, kunjungi halaman daftar Pembelian. Jika pesanan tidak dikirimkan, dana akan dikembalikan 100%";
    $tombol='<a href="'.$base_url.'/user/konfirmasi/'.$data_user[kd_user].'" class="btn btn-primary">Konfirmasi Pembayaran</a>';
  }
  $emailuser="";
  $namauser=""; 
  ...
  $sql_dtl_order=mysqli_query($conn, "SELECT barang.nama_barang, barang.gambar_barang, detail_order.harga, detail_order.qty, detail_order.total_harga FROM detail_order left join barang on barang.kd_barang=detail_order.kd_barang where kd_order='$data_konfirmasi_terupdate[kd_order]'");
  $isi_pesanan_konfirmasi="";
  ...
  while ($data_dtl_order=mysqli_fetch_array($sql_dtl_order)) {
    $subtotal_konfirmasi+=$data_dtl_order[total_harga];
    $isi_pesanan_konfirmasi=$isi_pesanan_konfirmasi.'<tr>
  ...
  $kode_pembayaran=$data_user[total]-$subtotal_konfirmasi;
  ...
  <div class="container">
    <div class="row justify-content-center my-5">
      <div class="col-7" style="margin:0 auto;">
        ...
        <h2>Invoice Pembayaran #'.$data_user[kd_order].' '.$atas.'</h2>
        <hr>
        ...
        <p>Berikut adalah penjelasan tagihan pembayaran:</p>
          <div class="card border-bottom-0">
            <div class="card-header bg-primary text-white text-center" style="width:100%;">
                <h4 class="card-title">Invoice #'.$data_user[kd_order].'</h4>
            </div>
          </div>
            ...
            <div class="card-body">
              <table class="table">
                <tr>
                  ...
                  <td>'.date("d F Y H:i", strtotime($data_user[tgl_order])).'</td>
                </tr>
                <tr>
              ...
              <table class="table border-left-0 border-right-0" style="margin-bottom: 0">
                <tbody>
                  <tr>
                    ...
                    <td class="font-weight-bold text-right" colspan="2">Rp.'.number_format($subtotal_konfirmasi).'</td>
                  </tr>
                  <tr>
                    <td width="400px;" >Kode Pembayaran <font class="text-muted" style="font-size: 10px;">(Hanya dibebankan kepada pembeli)</font</td>
                    <td class="font-weight-bold text-right" colspan="2">Rp.'.$kode_pembayaran.'</td>
                  </tr>
                </tbody>
                <tfoot class="bg-light">
                  <tr>
                    <td width="400px;" class="font-weight-bold">TOTAL PEMBAYARAN</td>
                    <td class="font-weight-bold text-right text-white bg-primary" colspan="2">Rp. '.number_format($data_user[total]).'</td>
                  </tr>   
                </tfoot>
              </table>
            </div>
          ...
          <p class="mt-4">
            ...
            '.$data_user[nama_pengiriman].'<br>
            '.$data_user[alamat_pengiriman].'<br>
...
$mail_konfirimasi = new PHPMailer;

//Tell PHPMailer to use SMTP
$mail_konfirimasi->isSMTP();

//Enable SMTP debugging
// 0 = off (for production use)
// 1 = client messages
// 2 = client and server messages
$mail_konfirimasi->SMTPDebug = 0;

//Ask for HTML-friendly debug output
$mail_konfirimasi->Debugoutput = 'html';

//Set the hostname of the mail server
$mail_konfirimasi->Host = 'smtp.gmail.com';
// use
// $mail_konfirimasi->Host = gethostbyname('smtp.gmail.com');
// if your network does not support SMTP over IPv6

//Set the SMTP port number - 587 for authenticated TLS, a.k.a. RFC4409 SMTP submission
$mail_konfirimasi->Port = 587;

//Set the encryption system to use - ssl (deprecated) or tls
$mail_konfirimasi->SMTPSecure = 'tls';

//Whether to use SMTP authentication
$mail_konfirimasi->SMTPAuth = true;

//Username to use for SMTP authentication - use full email address for gmail
$mail_konfirimasi->Username = "officialbengkelikan@gmail.com";

//Password to use for SMTP authentication
$mail_konfirimasi->Password = "bengkelikan11";

//Set who the message is to be sent from
$mail_konfirimasi->setFrom('noreply-officialbengkelikan@gmail.com', 'Bengkel Ikan');

//Set an alternative reply-to address
$mail_konfirimasi->addReplyTo('noreply-officialbengkelikan@gmail.com', 'Bengkel Ikan');

//Set who the message is to be sent to
$mail_konfirimasi->addAddress($emailuser, $nama);

//Set the subject line
$mail_konfirimasi->Subject = 'Konfirmasi Pembayaran - Invoice No #'.$data_konfirmasi[kd_order].' - Bengkel Ikan';

//Read an HTML message body from an external file, convert referenced images to embedded,
//convert HTML into a basic plain-text alternative body
$mail_konfirimasi->msgHTML($msg);

//Replace the plain text body with one created manually
//$mail_konfirimasi->AltBody = 'scdsddsdsd';

//Attach an image file
//$mail_konfirimasi->addAttachment('images/phpmailer_mini.png');

//send the message, check for errors
if (!$mail_konfirimasi->send()) {
    echo "Mailer Error: " . $mail_konfirimasi->ErrorInfo;
}
