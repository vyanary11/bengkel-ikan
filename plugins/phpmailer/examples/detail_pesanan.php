<?php
  $sql_orders=mysqli_query($conn, "SELECT * FROM orders left join user on user.kd_user=orders.kd_user WHERE kd_order='$_GET[kdorder]'");
  $data_orders=mysqli_fetch_assoc($sql_orders);
  $sql_dtl_order=mysqli_query($conn, "SELECT * FROM detail_order left join barang on barang.kd_barang=detail_order.kd_barang left join user on user.kd_user=barang.kd_user where kd_order='$_GET[kdorder]' and detail_order.kd_barang='$_GET[kdbarang]'");
  $data_dtl_order=mysqli_fetch_assoc($sql_dtl_order);
  if($_GET[s]==1){
    $subject="Hore! Pesanan NO. INVOICE #".$data_orders[kd_order]." dengan nama barang ".ucwords($data_dtl_order[nama_barang])." sedang diproses";
    $atas="Sedang diproses";
    $tengah="Hore! Pesanan NO. INVOICE <strong>#".$data_orders[kd_order]."</strong> dengan nama barang <strong>".ucwords($data_dtl_order[nama_barang])."</strong> sedang diproses oleh seller. Terima kasih atas kepercayaan kamu berbelanja di bengkelikan.com.";
    ...
    $emailuser=$data_orders[email];
    $namauser=$data_orders[nama];
  }else if($_GET[s]==2){
    $subject="Hore! Pesanan NO. INVOICE #".$data_orders[kd_order]." dengan nama barang ".ucwords($data_dtl_order[nama_barang])." dibatalkan";
    $atas="dibatalkan";
    $tengah="Maaf! Pesanan NO. INVOICE <strong>#".$data_orders[kd_order]."</strong> dengan nama barang <strong>".ucwords($data_dtl_order[nama_barang])."</strong> dibatalkan oleh seller. Terima kasih atas kepercayaan kamu berbelanja di bengkelikan.com.";
    $bawah="Untuk memantau status transaksimu, kunjungi halaman detail pesanan. Jika pesanan tidak dikirimkan, dana akan dikembalikan 100%";
    $emailuser=$data_orders[email];
    $namauser=$data_orders[nama];
  }else if($_GET[s]==3){
    $subject="Hore! Pesanan NO. INVOICE #".$data_orders[kd_order]." dengan nama barang ".ucwords($data_dtl_order[nama_barang])." Telah dikirim";
    $atas="Telah dikirim";
    $tengah="Hore! Pesanan NO. INVOICE <strong>#".$data_orders[kd_order]."</strong> dengan nama barang <strong>".ucwords($data_dtl_order[nama_barang])."</strong> Telah dikirim oleh seller. Terima kasih atas kepercayaan kamu berbelanja di bengkelikan.com.";
    $bawah="Untuk memantau status transaksimu, kunjungi halaman detail pesanan. Jika pesanan tidak dikirimkan, dana akan dikembalikan 100%";
    $emailuser=$data_orders[email];
    $namauser=$data_orders[nama];
  }else if($_GET[s]==4){
    $subject="Hore! Pesanan NO. INVOICE #".$data_orders[kd_order]." dengan nama barang ".ucwords($data_dtl_order[nama_barang])." Telah diterima";
    $atas="Telah diterima";
    $tengah="Maaf! Pesanan NO. INVOICE <strong>#".$data_orders[kd_order]."</strong> dengan nama barang <strong>".ucwords($data_dtl_order[nama_barang])."</strong> Telah diterima oleh pembeli. Terima kasih atas kepercayaan kamu berbelanja di bengkelikan.com.";
    $bawah="Untuk memantau status transaksimu, kunjungi halaman detail pesanan. Jika pesanan tidak dikirimkan, dana akan dikembalikan 100%";
    $emailuser=$data_dtl_order[email];
    $namauser=$data_dtl_order[nama];
  }
  $tombol='<a href="'.$base_url.'/user/detail-pesanan='.$data_orders[kd_order].'/'.$data_orders[kd_user].'" class="btn btn-primary">Lihat Detail Pesanan</a>';
  ...
  <div class="container">
    <div class="row justify-content-center my-5">
      <div class="col-7" style="margin:0 auto;">
        ...
        <h2>Invoice Pembayaran #'.$data_orders[kd_order].' '.$atas.'</h2>
        <hr>
        <p class="text-justify">Hai '.$data_orders[nama_lengkap].',<br>
        '.$tengah.'</p><br>
        <p class="text-justify">'.$bawah.'</p>
        <br>
        <p>Berikut adalah penjelasan tagihan pembayaran:</p>
          <div class="card border-bottom-0">
            <div class="card-header bg-primary text-white text-center" style="width:100%;">
                <h4 class="card-title">Invoice #'.$data_orders[kd_order].'</h4>
            </div>
          </div>
            ...
            <div class="card-body">
              <table class="table">
                <tr>
                  ...
                  <td>'.date("d F Y H:i", strtotime($data_orders[tgl_order])).'</td>
                </tr>
                <tr>
                  <td class="font-weight-bold">Pembeli</td>
                  <td>'.$data_orders[nama_lengkap].'</td>
                </tr>
              </table>
            </div>
          </div>
          ...
          <p class="mt-4">
            ...
            '.$data_orders[nama_pengiriman].'<br>
            '.$data_orders[alamat_pengiriman].'<br>
            No. Telp: '.$data_orders[no_telp].'
          </p>
...
$mail->Subject = ''.$subject.' - Bengkel Ikan';
