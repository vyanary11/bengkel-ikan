<?php 
	$sql=mysqli_query($conn, "SELECT * FROM gizi_ikan where kd_gizi='$_GET[id]'");
	$data=mysqli_fetch_assoc($sql);
?>
<div class="card mb-3">
	<div class="card-header">
		<i class="fa fa-edit"></i> Edit Gizi Ikan
	</div>
	<form method="post" action="<?php echo $base_url ?>/dashboard/bin/gizi/crud.php?id=<?php echo $_GET[id] ?>">
		<div class="card-body">
			<div class="row">
				<div class="col-lg-4">
						...
						<select required data-plugin="select2" style="width: 100%;" class="form-control" name="ikan">
							...
							<?php 
								...
								while($data_ikan=mysqli_fetch_assoc($sql_ikan)){
									...
									?>
										<option <?php if(mysqli_num_rows($sql_gizi)==1){ echo "disabled"; } ?> <?php if($data[kd_ikan]==$data_ikan[kd_ikan]){ echo "selected"; } ?> value='<?php echo $data_ikan[kd_ikan] ?>'><?php echo $kat ?> <?php if(mysqli_num_rows($sql_gizi)==1){ echo "( Sudah Terdata )"; } ?> </option>	
									<?php	
								}
							?>
						</select>
